...
public class SwerveModule {
  ...
  public SwerveModule(
    ...
    driveEncoder.setPositionConversionFactor(
        2 * Math.PI * kWheelRadius * RobotMap.Swerve.SWERVE_CONVERSION_FACTOR / kEncoderResolution);
    driveEncoder.setVelocityConversionFactor(
        2 * Math.PI * kWheelRadius * RobotMap.Swerve.SWERVE_CONVERSION_FACTOR / kEncoderResolution);

    m_turningPIDController.enableContinuousInput(-Math.PI, Math.PI);

    // turningEncoder.setPosition(turningEncoder.getPosition().getValueAsDouble());
  ...
  public SwerveModulePosition getPosition() {
    return new SwerveModulePosition(
        ...
        Rotation2d.fromDegrees(turningEncoder.getAbsolutePosition().getValueAsDouble() % 360));
  }
  ...
  public void setDesiredState(SwerveModuleState desiredState) {
    ...
    SwerveModuleState state = SwerveModuleState.optimize(desiredState,
        Rotation2d.fromDegrees(turningEncoder.getPosition().getValueAsDouble() % 360));

    // Calculate the drive output from the drive PID controller.
    final double driveOutput = m_drivePIDController.calculate(
        driveEncoder.getVelocity() * 2 * Math.PI * RobotMap.Swerve.SWERVE_CONVERSION_FACTOR * kWheelRadius,
    ...
    final double turnOutput = m_turningPIDController
        .calculate(Math.toRadians(turningEncoder.getPosition().getValueAsDouble() % 360), state.angle.getRadians());
